...
# Then we generate all combinations of indices (in cmbsa)
# for all entries in crc, and their lengths in cmbs.
...
# we need to check if it conflicts with either possn0,
...
# 3. with possn0 if corresponding entry in combination
#    also exists in possn0 (same as possn)
...
# filled. As it happens with 13th column for txin1.
...
# Another optimization possible in this code is to store
# cmbsa for each row and eliminate all invalid combinations
# according to possn0 before main loop itself, this will
# cause it to not check those again and again
...
# Generating all possible combination indices
cmbsa = {}
cmbs = {}
for c in set(crc):
    c1 = cmbsl(ncj, c)
...
while not chk:
    ...
    if tk[k] >= cmbs[crc[k]]:
        ...
        continue
    chk = True
    # Checking if new combination has any conflict with possn0
    for c in cmbsa[crc[k]][tk[k]]:
        if dgs[k,c] in possn0[c]:
            chk = False
            break
    if not chk:
        continue
