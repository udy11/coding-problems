...
def fqcr():
    ...
    for j in range(r + 1, qn):
        cc = 0
        for k in range(qm):
            if ss[k] > 0 and qt[j][k] == ss[k]:
                cc += 1
        if cc > qt[j][-1]:
            return False
    c2 = qt[r][-1]
...
qs11 = '''3847439647293047 ;1 correct
